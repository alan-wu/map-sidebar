    ...
    createPlotItems: function () {
      if (this.entry.plots) {
        this.entry.plots.forEach((plot) => {
          ...
          const sourceUrl =
            filePathPrefix + plot.dataset.path + this.getS3Args()
    ...
    createScaffoldItems: function () {
      if (this.entry.scaffolds) {
        ...
        this.entry.scaffolds.forEach((scaffold, i) => {
          ...
          let action = {
            label: capitalise(this.label),
            resource: `${this.envVars.API_LOCATION}s3-resource/${
              this.datasetId
            }/${this.datasetVersion}/files/${filePath}${this.getS3Args()}`,
      ...
      if (
        ...
        let contextIndex =
          ...
          this.entry.scaffolds.length
            ? scaffoldIndex
            : 0
        return `${this.envVars.API_LOCATION}s3-resource/${this.datasetId}/${
          this.datasetVersion
        }/files/${
          this.entry.contextualInformation[contextIndex]
        }${this.getS3Args()}`
      }
    },
  },
